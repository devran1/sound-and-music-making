...
for i in a:
    ...
    for_shuffle =signal_list
    random.shuffle(for_shuffle)
        
    df=pd.DataFrame(for_shuffle)    
    ...
    """        
        ...
        divided_data=[]
        for k in range(int(Time[-1])+1):
            
            k=k+1
            
            divided_data += [signal[(k-1)*int(len(signal)/Time[-1]):k*int(len(signal)/Time[-1])]]
...
#            df=pd.DataFrame(signal_list)
            ...
            #df.to_pickle(f"music-data/original-{i}-.pickle") #writes the last one
        ...
        #ordered data for ai
        with_order=[]
        for l,j in enumerate(divided_data):
            o_data=[l,j]
#            with_order +=[o_data]
